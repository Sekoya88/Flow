        ...
        "proposals": [
            {
                ...
                **({"execution_id": str(r["execution_id"])} if r.get("execution_id") is not None else {}),
            }
            for r in rows
        ]
    }
    ...
    async with repo._pool.acquire() as conn:
        async with conn.transaction():
            updated = await conn.fetchrow(
                "UPDATE proposals SET status = $3 WHERE id = $1 AND workspace_id = $2 RETURNING id",
                proposal_id,
                ws,
                body.status,
            )
            ...
            if body.status == "approved":
                ...
                proposal_rec = await conn.fetchrow("SELECT body FROM proposals WHERE id = $1", proposal_id)
                if proposal_rec:
                    ...
                    if skill_candidate_id:
                        ...
                        if sibling:
                            await conn.execute(
                                "UPDATE agent_skills SET active = false WHERE agent_id = $1 AND name = $2 AND active = true",
                                sibling["agent_id"],
                                sibling["name"],
                            )
                            await conn.execute(
