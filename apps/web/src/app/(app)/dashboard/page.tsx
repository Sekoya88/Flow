    ...
    <div className="w-full space-y-12 animate-fade-in">
      ...
      <section className="relative overflow-hidden rounded-[6px] border border-flow-800 bg-card p-6 sm:p-8 animate-slide-up">
        ...
        <div className="relative z-10 flex flex-col gap-2">
          <Badge
            variant="outline"
            className="w-fit border-flow-violet/35 bg-flow-violet/10 font-mono text-[10px] uppercase tracking-wider text-flow-violet"
          >
            Workspace Overview
          </Badge>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {TILES.map(({ key, label, href }, i) => (
          ...
          >
            <div
              className={cn(
                "flow-card relative flex min-h-[120px] flex-col overflow-hidden rounded-[6px] p-5 transition-all hover:border-flow-violet/50 hover:shadow-md",
      ...
      <div className="grid gap-4 md:grid-cols-12">
        ...
        <div className="flow-card flex flex-col items-center justify-center gap-6 rounded-[6px] p-8 md:col-span-5 text-center animate-slide-up [animation-delay:200ms]">
          <FlowMark className="text-flow-violet" size={48} />
                ...
                {recent.map((r) => (
                  <li key={r.id} className="flex items-center gap-3 px-6 py-3.5 transition-colors hover:bg-muted/10">
                    {r.status === "completed" ? (
                      ...
                      <Clock className="h-4 w-4 shrink-0 animate-pulse text-flow-violet" />
                    )}
        ...
        <div className="flow-card flex flex-col overflow-hidden rounded-[6px] md:col-span-6 animate-slide-up [animation-delay:320ms]">
          <div className="flex items-center gap-2 border-b border-flow-800 bg-muted/20 px-6 py-4">
            ...
            {proposals.length > 0 && (
              <Badge
                variant="outline"
                className="ml-auto h-5 rounded-full border-flow-thinking/30 bg-flow-thinking/10 px-2 py-0 font-mono text-[10px] text-flow-violet"
              >
            ...
            {proposals.length === 0 ? (
              ...
              <ul className="space-y-3">
                ...
                {proposals.length > 4 && (
                  <Link
                    href="/proposals"
                    className="mt-3 block text-center font-mono text-[11px] text-flow-violet transition-colors hover:text-flow-violet/80"
                  >
        ...
        <div className="flow-card flex flex-col overflow-hidden rounded-[6px] md:col-span-6 animate-slide-up [animation-delay:380ms]">
          <div className="flex flex-col space-y-1 border-b border-flow-800 bg-muted/20 px-6 py-4">
            <div className="flex items-center gap-2">
              <Database className="h-4 w-4 text-flow-violet" aria-hidden />
            ...
            <Accordion multiple={false} className="w-full">
              {POSTGRES_MODEL.map((row) => (
                <AccordionItem key={row.table} value={row.table} className="border-b-border/40">
                  ...
                  <AccordionContent>
                    <ul className="grid grid-cols-2 gap-2 border-l-2 border-flow-violet/25 pl-4 font-mono text-[10px] text-muted-foreground">
                      {row.columns.map((col) => (
